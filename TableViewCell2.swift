...
class TableViewCell2: UITableViewCell, UICollectionViewDataSource {
    ...
    var selectedPost2: Posts?
    
    @IBOutlet weak var theCollectionView: UICollectionView!
    
    func collectionView(collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        let number = posts2.count
        return number
    }
    
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        let collectionViewCell = collectionView.dequeueReusableCellWithReuseIdentifier("CollectionViewCell2", forIndexPath: indexPath) as! CollectionViewCell2
        
        let imageUrlString: String = posts2[indexPath.row].postThumbnailUrlString
        ...
        collectionViewCell.postImageView.af_setImageWithURL(imageUrl)
        collectionViewCell.postTitleLabel.text = posts2[indexPath.row].postTite
        
        //give blur view a round corner to match the images' corner
        let path = UIBezierPath(roundedRect:collectionViewCell.bounds, byRoundingCorners:[.BottomRight, .BottomLeft, .TopLeft, .TopRight], cornerRadii: CGSize(width: 20, height:  20))
    ...
    func scrollToNextCell(){
        ...
        if theCollectionView.contentSize.width <= theCollectionView.contentOffset.x + cellSize.width
        {
            ...
            theCollectionView.scrollRectToVisible(CGRectMake(contentOffset.x + cellSize.width - 64, contentOffset.y, cellSize.width - 64, cellSize.height), animated: true);
